...
<script type="text/javascript">
	...
	function applyOrder() {
		...
		window.location.href = "/cart/catrToOrder?sendId=" + midd
				+ "&returnUrl=" + encodeURIComponent(window.location.href);
	}
</script>
</head>
	...
	<div class="concent">
		...
		<div class="paycont">
			...
			<div class="body1">
				...
				<c:forEach items="${proShops}" var="pro">
					<table class="order_commodity order_commodity_w">
						<thead>
							<tr class="firm">
								<td colspan="7">
									<div class="firm_w">${pro.phName}</div>
								</td>
							</tr>
						</thead>
						<tbody>
							<c:forEach items="${pro.buyerItems}" var="pb">
								<tr class="firm_list">
									<td><img src="${pb.product.pic}" width="60px"
										height="60px" /></td>
									<td><span class="inventory_cue_b">${pb.product.name}<span
											class="inventory_cue"><c:if test="${!pb.isHave}">库存不足</c:if></span></span></td>
									<td><span class="specifications">${pb.sku.spec}</span></td>
									<td><span class="univalent">${pb.sku.price}</span></td>
									<td><span class="number">${pb.amount}</span></td>
									<td><span class="subtotal">${pb.amount * pb.sku.num}</span></td>
								</tr>
							</c:forEach>
						</tbody>
						<tfoot>
							...
							<tr>
								<td colspan="7">
									<div style="float: right;">
										<span>${pro.msg}</span> 运费 <b class="red total_y yun">￥</b> <b
											class="red total_w yun">${pro.mailFee}</b>
									</div>
								</td>
							</tr>
						</tfoot>
			...
			<div class="order_last_box">
				...
				<c:choose>
					...
					<c:otherwise>
						<input type="button" name="" class="order_submit1" value="提交订单" />
					</c:otherwise>
				</c:choose>
			</div>
		</div>
